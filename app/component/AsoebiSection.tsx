import { CONTACT_PHONE, asoebiOptions, paletteCards } from "../types/constant";
import { LinkPill, SectionHeading } from "./ui";

export default function AsoebiSection() {
  return (
    <section id="style" className="relative overflow-hidden py-20 text-white sm:py-24">
      <div className="absolute inset-0 bg-[linear-gradient(135deg,#582035_0%,#9d4e66_56%,#d1a860_140%)]" />
      <div className="pattern-lattice absolute inset-0 opacity-45" />
      <div className="pattern-petals absolute inset-0 opacity-35" />
      <div className="float-bloom pointer-events-none absolute -left-10 top-20 h-40 w-40 rounded-full bg-white/10 blur-3xl" />
      <div className="float-bloom-delayed pointer-events-none absolute right-0 top-12 h-52 w-52 rounded-full bg-[#f3c8d0]/22 blur-3xl" />

      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 xl:grid-cols-[0.9fr_1.1fr] xl:items-start">
          <div>
            <SectionHeading
              eyebrow="Dress Code & Asoebi"
              title="Come dressed in softness, joy, and celebration"
              subtitle="The palette has been arranged to feel rich, flattering, and consistent across both days."
              tone="light"
            />

            <div className="mt-6 rounded-[2rem] border border-white/14 bg-white/10 p-6 backdrop-blur">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#f5dfb0]">
                Fabric contact
              </p>
              <p className="mt-4 text-base leading-8 text-white/82">
                For asoebi reservations, gele details, or style questions, please reach out
                directly using the contact below.
              </p>
              <div className="mt-5">
                <LinkPill href={`tel:${CONTACT_PHONE}`} variant="ghost">
                  Call {CONTACT_PHONE}
                </LinkPill>
              </div>
            </div>
          </div>

          <div className="grid gap-4 sm:grid-cols-3">
            {paletteCards.map((card) => (
              <div
                key={card.name}
                className="rounded-[1.9rem] border border-white/14 bg-white/10 p-5 backdrop-blur"
              >
                <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[#f5dfb0]">
                  {card.name}
                </p>
                <h3 className="mt-3 font-serif text-3xl leading-tight text-white">
                  {card.label}
                </h3>
                <div className="mt-5 flex items-center gap-3">
                  {card.colours.map((colour) => (
                    <span
                      key={colour}
                      className="h-11 w-11 rounded-full border border-white/22 shadow-[inset_0_1px_0_rgba(255,255,255,0.3)]"
                      style={{ backgroundColor: colour }}
                    />
                  ))}
                </div>
                <p className="mt-5 text-sm leading-7 text-white/82">{card.note}</p>
              </div>
            ))}
          </div>
        </div>

        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {asoebiOptions.map((option) => (
            <div
              key={option.label}
              className="rounded-[1.9rem] border border-white/14 bg-white/10 p-6 backdrop-blur"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#f5dfb0]">
                Asoebi option
              </p>
              <h3 className="mt-4 font-serif text-3xl leading-tight text-white">
                {option.label}
              </h3>
              <p className="mt-4 text-2xl font-semibold text-[#fff0c9]">{option.price}</p>
              <p className="mt-4 text-sm leading-7 text-white/82">{option.note}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
